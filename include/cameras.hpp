...
public:
    ...
    cameras(char cam_type) : _intv{0}, _last_read(std::chrono::high_resolution_clock::now()), _idx(0){
        std::fill_n(this->_intv, sizeof(this->_intv) / sizeof(this->_intv[0]), 0x7f7f7f7f);
        cv::utils::logging::setLogLevel(cv::utils::logging::LOG_LEVEL_FATAL);                   // 设置opencv打印日志的等级, 不再打印warn信息
        this->_cam_type = 'x';
        //------------------------------设置摄像头基本参数------------------------------
...
private:
    bool type_is_useful(char cam_type) {
...
private:
    ...
    char _cam_type;                                     // 摄像头的类型: h - hik摄像头; i - 工业摄像头; d - 深度摄像头
